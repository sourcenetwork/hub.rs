...
impl AcpModule {
    ...
    ///
    /// Flow:
    ///   1. Build `RecordMetadata` from execution context:
    ///      ```text
    ///      creation_ts = self.block_ctx.timestamp
    ///      tx_hash     = self.tx_ctx.tx_hash
    ///      tx_signer   = self.tx_ctx.signer
    ///      owner_did   = creator.to_string()
    ///      ```
    ///   2. Call `engine.create_policy(creator, policy, marshal_type, metadata)`
    ///      - Engine parses `policy` string according to `marshal_type`
    ///        (ShortYaml → YAML parser, ShortJson → JSON parser)
    ///      - Engine validates schema: resource names unique, relation types
    ///        reference valid resources/actors, permission expressions reference
    ///        defined relations
    ///      - Engine generates autoincrement policy ID
    ///      - Engine stores the `PolicyRecord` internally (zanzibar policy store)
    ///   3. Return the `PolicyRecord` from the engine response
    ///
    /// Reads: engine internal state (schema validation)
    ...
    ///     schema validation (invalid resources, relations, or permissions)
    ///
    /// The policy ID is generated by the engine (autoincrement integer
    /// returned as string), not by this method. The caller receives it
    /// back inside `PolicyRecord.policy.id`.
    ...
    ///
    /// Flow:
    ///   1. Build `RecordMetadata` from execution context (same as create_policy)
    ///   2. Call `engine.edit_policy(creator, policy_id, policy, marshal_type, metadata)`
    ///      - Engine fetches existing policy by `policy_id`
    ///      - Engine verifies `creator` DID matches the policy's owner
    ///      - Engine parses and validates new policy definition
    ///      - Engine removes any relationships that reference resources/relations
    ///        no longer defined in the new schema
    ...
    /// Ctx: block_ctx + tx_ctx (for RecordMetadata)
    ///
    /// Errors:
    ...
    ///        - If object registered and commitment is newer: amends ownership,
    ///          creates AmendmentEvent at `"amendment_event/"` prefix
    ...
    ///        - Validates creator DID matches event's previous_owner
    ...
    ///   9. Replay check: compute payload_id = SHA256(payload)
    ...
    ///
    /// Flow:
    ///   1. Call `engine.get_policy(id)`
    ///   2. If not found, return `AcpError::PolicyNotFound`
    ///   3. Extract `RecordMetadata` from engine's SuppliedMetadata blob
    ...
    ///   2. Extract the ID from each record
    ///   3. Return the IDs as Vec<String>
    ///
    ...
    ///
    /// Flow:
    ///   1. Call `engine.filter_relationships(policy_id, selector)`
    ///      - `ObjectSelector`: Exact(Object), Wildcard, or ResourcePredicate(resource)
    ///      - `RelationSelector`: Exact(relation), or Wildcard
    ///      - `SubjectSelector`: Exact(Subject), or Wildcard
    ///   2. For each result, extract `RecordMetadata` from engine metadata blob
    ///   3. Return Vec<RelationshipRecord>
    ///
    /// Reads: engine relationship store (filtered query)
    ///
    ...
    ///
    /// Flow:
    ///   1. Call `engine.verify_access_request(policy_id, access_request)`
    ///      - Evaluates ALL operations against the Zanzibar graph
    ///   2. Return true if all operations pass, false otherwise
    ///
    ...
    ///
    /// Flow:
    ///   1. Call `engine.validate_policy(policy, marshal_type)` (stateless parse)
    ///   2. If valid: return (true, "", parsed Policy)
    ///   3. If invalid: return (false, error_description, Policy::default())
    ///
    /// Reads: nothing (stateless validation)
    ///
    /// Invalid policy is NOT an error — it returns (false, msg, ...).
    /// Only internal failures return Err.
    #[allow(unused_variables)]
    ...
    ///   3. Return the decision (or None-equivalent if not found)
    ///
    /// Reads: access decision store (`"access_decision/"` prefix)
    ///
    /// Not found returns Ok with a default/empty decision in Go.
    /// Consider returning Option<AccessDecision> or AcpError::NotFound.
    #[allow(unused_variables)]
    ...
    ///
    /// Flow:
    ///   1. Read from KV prefix `"commitment/"` using the u64 ID
    ///   2. Deserialize as `RegistrationsCommitment`
    ///
    /// Reads: commitment store (`"commitment/"` prefix, keyed by u64 ID)
    ///
    /// Errors:
    ...
    ///
    /// Flow:
    ///   1. Query commitment store secondary index on `commitment` field
    ///   2. Materialize all matching `RegistrationsCommitment` records
    ///
    /// Reads: commitment store (`"commitment/"` prefix, `commitment` index)
    ///
    /// Returns empty vec if no matches (not an error).
