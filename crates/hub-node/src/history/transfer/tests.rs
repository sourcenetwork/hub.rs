...
pub(in crate::history) fn revision(height: u64, parent: BlockId) -> (Block, Vec<ExecutionReceipt>) {
    ...
    let receipts = if height == 0 {
        ...
        block.txs.push(Tx::new(vec![height as u8].into()));
...
async fn authenticated_reverse_import_resumes_and_requires_matching_state_recovery() {
    ...
    let (_, wrong_key) = certify(&third, 8);
    {
        ...
        assert!(
            target
                .import_record(&third_bytes, HistoryLimits { logs: 0, ..limits })
                .is_err()
        );
        for end in [0, 4, third_bytes.len() / 2, third_bytes.len() - 1] {
            assert!(target.import_record(&third_bytes[..end], limits).is_err());
        }
        target.import_record(&third_bytes, limits).unwrap();
    ...
    assert!(target.import_record(&third_bytes, limits).is_err());
    ...
    target.import_record(&second_bytes, limits).unwrap();
