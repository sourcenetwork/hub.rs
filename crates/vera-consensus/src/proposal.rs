...
use vera_domain::{Block, ConsensusContext, StateRoot, Tx};
use vera_executor::{BlockContext, BlockExecutor};
...
fn block_context(height: u64, timestamp: u64, prevrandao: B256) -> BlockContext {
    let header = Header {
        number: height,
        timestamp,
        gas_limit: vera_config::DEFAULT_GAS_LIMIT,
    ...
    E: BlockExecutor<S, Tx = Bytes>,
{
    ...
    pub fn build_proposal(
        ...
        let block = Block {
            ...
            db_targets: vera_domain::DbTargets::default(),
        };
...
mod tests {
    ...
    use vera_executor::ExecutionOutcome;
    ...
    impl vera_traits::StateDbRead for MockStateDb {
        async fn nonce(&self, _address: &Address) -> Result<u64, vera_traits::StateDbError> {
            Ok(0)
        }

        async fn balance(&self, _address: &Address) -> Result<U256, vera_traits::StateDbError> {
            Ok(U256::ZERO)
        }

        async fn code_hash(&self, _address: &Address) -> Result<B256, vera_traits::StateDbError> {
            Ok(B256::ZERO)
        }

        async fn code(&self, _code_hash: &B256) -> Result<Bytes, vera_traits::StateDbError> {
            Ok(Bytes::new())
        }
        ...
        async fn storage(
            ...
            _address: &Address,
            _slot: &U256,
        ) -> Result<U256, vera_traits::StateDbError> {
            Ok(U256::ZERO)
        }
    }

    impl vera_traits::StateDbWrite for MockStateDb {
        async fn commit(&self, _changes: ChangeSet) -> Result<B256, vera_traits::StateDbError> {
        ...
        async fn compute_root(
            &self,
            _changes: &ChangeSet,
        ) -> Result<B256, vera_traits::StateDbError> {
            Ok(B256::repeat_byte(0x42))
        }
    ...
    impl vera_traits::StateDb for MockStateDb {
        async fn state_root(&self) -> Result<B256, vera_traits::StateDbError> {
            Ok(self.root)
        }
    }
    ...
    impl BlockExecutor<MockStateDb> for MockExecutor {
        ...
        fn execute(
            ...
            txs: &[Self::Tx],
        ) -> Result<ExecutionOutcome, vera_executor::ExecutionError> {
            Ok(ExecutionOutcome {
        ...
        fn validate_header(&self, _header: &Header) -> Result<(), vera_executor::ExecutionError> {
            Ok(())
        }
    }
    ...
    fn parent_block() -> Block {
        Block {
            ...
            parent: vera_domain::BlockId(B256::ZERO),
            height: 0,
